...
namespace Firefly
{
		...
		m_physicalDevice(device->GetPhysicalDevice())
	{
		...
		m_swapchain = VulkanUtils::CreateSwapchain(m_device, m_physicalDevice, surface->GetSurface(), m_swapchainData);
		...
		for (size_t i = 0; i < m_images.size(); i++)
			m_imageViews[i] = VulkanContext::GetSingleton()->CreateImageView(m_images[i], mipLevels, m_swapchainData.imageFormat, vk::ImageAspectFlagBits::eColor);
	}
